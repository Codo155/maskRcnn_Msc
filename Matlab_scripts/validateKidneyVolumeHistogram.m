                ...
                for t=5:5
                    ...
                    if ~isnan(dice_res) && all(size(labels) >0)
                        for counter= 1:20
                            ...
                            if dice_res<=ma && dice_res>mi
                                dice_results(counter,1)=dice_results(counter,1)+kidneyPixel;
                            end
                        end                        
                    end
                    ...
                    if ~isnan(rvd) && all(size(labels) >0)
                        for counter= 1:20
                            ...
                            mi= ((counter-1)*5.5);
                            if rvd<=ma && rvd>mi
                                rvd_results(counter,1)=rvd_results(counter,1)+kidneyPixel;
                            end
                        end                         
                    end
                    ...
                    if ~isnan(jaccard_res) && all(size(labels) >0)
                        for counter= 1:20
                            ...
                            if jaccard_res<=ma && jaccard_res>mi
                                jaccard_results(counter,1)=jaccard_results(counter,1)+kidneyPixel;
                            end
                        end                         
                    end
                    if ~isnan(accuracy) && all(size(labels) >0)
                        for counter= 1:20
                            ...
                            if accuracy<=ma && accuracy>mi
                                accuracy_results(counter,1)=accuracy_results(counter,1)+kidneyPixel;
                            end
                       end                         
                    end
                end
